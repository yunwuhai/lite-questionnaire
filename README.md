...
| `rating` | 评分 | 滑块条 + 表情量表 + 文字注释 | 提交默认中间值并前进 |

> **注意**：所有问题均为必答。`confirm` 和 `rating` 具有默认值（是 / 中间值），无需进入编辑态即可直接 Enter 提交。`text` 类型留空按 Enter 会显示错误提示，不可跳过。←/→ 切换问题时自动提交当前草稿（有值则保存，无值则标红）。
...
| 字段 | 类型 | 默认 | 说明 |
|------|------|------|------|
| `range` | { min, max } | - | 评分范围（闭区间） |
| `showEmoji` | boolean | false | 显示表情量表（😡😟😐😊😍） |
| `annotations` | Record&lt;number, string&gt; | - | 数值到文字注释的映射 |

> 默认值为范围中间值（如 1-5 默认 3）。编辑态 ←/→ 或数字键调整，Enter 提交。
