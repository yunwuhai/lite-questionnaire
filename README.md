...
- **↑↓** 移动光标
- Esc 取消
- Space 在 "Type something." 上先清除 radio 选中，再进入编辑器，提交后立即保存
...
**多问题模式：**
- **Tab** 保存当前选择 → 跳到下一个未回答的问题（跳过已回答的）
- **Shift+Tab** 同上，反向查找未回答的问题
- **← →** 保存当前选择 → 相邻跳转（包括 Submit 页）
- **Enter** 保存当前选择并校验（多选必须至少选一项）→ 前进到下一题
- 切换 Tab/方向键时自动保存草稿，返回时恢复已选状态和自定义文本
- 全部答完后 Tab 跳转到 "Submit" 汇总页
- Submit 页要求所有问题均有非空答案才能提交
- 每个问题独立作答，互不干扰
